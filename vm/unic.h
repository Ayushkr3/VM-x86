...
inline void ReadRegUni(UnicornData* ud) {
	...
	err = uc_reg_read(ud->uc, UC_X86_REG_LDTR, &ud->cpu->LDT);
	err = uc_reg_read(ud->uc, UC_X86_REG_IDTR, &ud->cpu->IDT);
	
}

inline void WriteRegUni(UnicornData* ud) {
	uc_err err = uc_reg_write_batch(ud->uc, ud->cpu->GetRegOrder(), (void**)&ud->cpu->Reg, 21);
	err = uc_reg_write(ud->uc, UC_X86_REG_GDTR, &ud->cpu->GDT);
	err = uc_reg_write(ud->uc, UC_X86_REG_LDTR, &ud->cpu->LDT);
	err = uc_reg_write(ud->uc, UC_X86_REG_IDTR, &ud->cpu->IDT);
}
